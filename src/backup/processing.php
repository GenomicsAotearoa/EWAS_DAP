...
shell_exec("LD_LIBRARY_PATH=/lib64:/usr/lib64:/usr/lib64/R/lib nohup Rscript /opt/lampp/htdocs/ewas/ewas_pip/$ip/run.R $ip $norm_method $email $snp $xy_chr $cr_probes $pheno $design_matrix $norm_method_selection $array_type $deconv $com_group $array_corr $batch_corr $norm_name > $ip/file.out 2> $ip/file.error < /dev/null &");
