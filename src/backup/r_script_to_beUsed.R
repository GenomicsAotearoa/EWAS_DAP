...
print ("No correction required")
	...
	for (i in 1:len) {
		...
		adj		<-	tryCatch( {		
			...
			if (adj == 0) {
				covar_temp		<-	str_replace(covars[,i], "myLoad\\$pd\\$", "")
				output			<-	paste(output, "\n\t\tAdjusted not successful for : ", covar_temp, sep="")
			}	
			if (adj != 0) {
...
tryCatch( {
	...
	if(com_type == "1") {
			...
			DMPs		<-	champ.DMP ( beta = m_values, pheno = com_group, arraytype = arraytype)
			DMP_groups	<-	names(DMPs)			
			
			output          <-      paste(output, "\t\t\tAnalysis type : Contineous variable", sep="")
		...
		error = function(c)   {  
		output		<-	paste(output, "\t\t\tDMP Identification failed", sep="")  
		},
		warning = function(c) {  
		output		<-	paste(output, "\t\t\tDMP Identification failed", sep="")  
		}
