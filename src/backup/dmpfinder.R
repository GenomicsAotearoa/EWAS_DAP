...
if(data_load_method == 1) {
	...
	output          <-      paste ( output, "\n\n\nNormalization method : ", normalize, sep="")
...
output          <-      paste(output, "Blood cell correction method: None", sep="")
}
...
output                  <-      paste(output, "\n\n\nConfonding factors adjustment module" ,sep="")
if (DM == "-") {
...
tryCatch( {
	if(com_type == "3") {			
			...
			output		<-	paste(output, "\t\t\tRank Regression on : ", pheno_temp, "\t\t\tAnalysis type : Contineous variable", sep="")
	...
	if(com_type == "2") {
				...
				result		<-	merge(x = annotation, y = DMPs, by = "Name", all.y = TRUE)
				 ...
				 output          <-      paste(output, "\t\t\tLinear Regression on : ", pheno_temp, "\t\t\tAnalysis type : Contineous variable", sep="")
	...
	if(com_type == "1")  {
			...
			DMPs		<-		dmpFinder(m_values, com_group_new, type = "categorical", qCutoff = 1, shrinkVar=TRUE)
			...
			result		<-		merge(x = annotation, y = DMPs, by = "Name", all.y = TRUE)
				...
				output		<-      paste(output, "\t\t\tF-Test Performed on : ", pheno_temp, "\t\t\tAnalysis type : Categorical variable", sep="")
					...
					colnames(result)[7]			<-		"P_VAL"	
					colnames(result)[2]			<-		"CHR"				
					colnames(result)[3]			<-		"MAPINFO"				
					colnames(result)[4]			<-		"gene"

					file           				<-      paste(foldername, "/DMP_categorical_var.csv", sep = "")
					write.csv (result, file)
				...
				result$logFC				<-		log2(result$f)
						
				comm						<-		intersect(result$Name, snps$V1)
				result$snps					<-		1
				mat							<-		match(comm, result$Name)
				result$snps[mat]			<-		2
				df_snps						<-		result[mat,]
				

				non_snp_DMPs		<-		setdiff(result$Name, snps$V1)
				mat					<-		match(non_snp_DMPs, result$Name)
				df_non_snps			<-		result[mat,]
				
				
				###################### code for interactive volcano plot ########################
				
				file						<-		paste ( foldername, "/graph_plotting_snps_1.csv", sep = "")				
				graph_data_snps				<-		paste ( "{x:", df_snps$logFC,
															",y:", df_snps$PValue,
															",name:'", df_snps$Name, "_", df_snps$gene, "'}",
															sep = "")								
				write.table(graph_data_snps, file, quote = TRUE, sep = ",", row.names = FALSE, col.names = FALSE)
				
				file						<-		paste ( foldername, "/graph_plotting_non_snps_1.csv", sep = "")				
				graph_data_non_snps			<-		paste ( "{x:", df_non_snps$logFC,
															",y:", df_non_snps$PValue,
															",name:'", df_snps$Name, "_", df_non_snps$gene, "'}",
															sep = "")								
				...
				#########Code for volcano plot starts here . . . ###########
				
				file	<-	paste(foldername, "/1_volcano_plot.jpg", sep = "")
				jpeg(file, width = 1800, height = 1200)	
				plot (  result$logFC, result$PValue, pch=19, col = c("BLUE", "RED")[result$snps], xlim=c(-1,1), xlab="Log2 Fold Change", ylab= "Log10 P-Value", bty="l"  )
				dev.off()
			...
			#HS		<-	champ.EpiMod  (  beta = norm, pheno = com_group, nseeds = 100, gamma = 0.5, sizeR.v = c(1,100), minsizeOUT = 10, PDFplot = TRUE, nMC = 1000, resultsDir = file, arraytype = arraytype)
